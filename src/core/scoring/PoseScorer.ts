...
interface BucketSnapshot {
  buckets: Buckets;
  /** Real angle mismatch per metric id, in degrees (mean over contributing joints). */
  angleErrs: Buckets;
  /** Real position mismatch per metric id, in meters (mean over mapped points). */
  distErrs: Buckets;
  aggregate: number;
}
...
const METRIC_DISTANCE_POINTS: Record<string, Array<{ mp: MpAnchor; joint: JointName }>> = {
  knee: [
    { mp: 25, joint: "lKnee" },
    { mp: 26, joint: "rKnee" },
  ],
...
function computeBuckets(
  ...
  const angleErrs: Buckets = {};
  const distErrs: Buckets = {};
