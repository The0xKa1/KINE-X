...
export interface JointSessionStat {
  id: string;
  name: string;
  avgScore: number;
  worstScore: number;
...
export type SessionPhase = "intro" | "mid" | "peak" | "outro";
...
export interface SessionSummary {
  ...
  worstPhase: SessionPhase;
  phaseAvgScores: Record<SessionPhase, number>;
  joints: JointSessionStat[];
}
...
const PHASES: SessionPhase[] = ["intro", "mid", "peak", "outro"];
const FRAME_EMIT_INTERVAL_MS = 120;

export class SessionRecorder {
  private joints = new Map<string, JointAccumulator>();
  private phaseStats: Record<SessionPhase, PhaseAccumulator> = {
    intro: { sum: 0, count: 0 },
    mid: { sum: 0, count: 0 },
    peak: { sum: 0, count: 0 },
    outro: { sum: 0, count: 0 },
  ...
  ingest(payload: ScoreUpdate): void {
    ...
    payload.metrics.forEach((metric: JointMetric) => {
  ...
  snapshot(): SessionSummary {
    ...
    const phaseAvg: Record<SessionPhase, number> = {
    ...
    let worstPhase: SessionPhase = "intro";
    ...
    const joints: JointSessionStat[] = Array.from(this.joints.values())
      .map((acc) => ({
        id: acc.id,
        name: acc.name,
...
function pickPhase(progress: number): SessionPhase {
...
function avgFor(bucket: PhaseAccumulator): number {
