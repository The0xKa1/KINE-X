...
export interface AvatarApplyPayload {
  seedId: string;
  name: string;
  avatarBinUrl: string;
}

export type AvatarFlowState = "empty" | "photo" | "building" | "done" | "error";
...
export interface AvatarImportFlowOptions {
  ...
  backendUrl: string;
  /** Target seed the avatar learns to move with (v1: always "ugc-squat"). */
  seedId: string;
  onReady(payload: AvatarApplyPayload): void;
  onEnter(payload: AvatarApplyPayload): void;
  onStateChange?(state: AvatarFlowState): void;
}
...
export class AvatarImportFlow {
  private options: AvatarImportFlowOptions;
  private file: File | null = null;
  private busy = false;
  private applied: AvatarApplyPayload | null = null;
  private pollTimer = 0;
  private mockTimer = 0;
  private readonly mock: boolean;

  constructor(options: AvatarImportFlowOptions) {
  ...
  private emitState(state: AvatarFlowState): void {
  ...
  private runMock(name: string): Promise<AvatarApplyPayload> {
  ...
  private async runReal(name: string): Promise<AvatarApplyPayload> {
    this.setStatus(`上传到 ${this.options.backendUrl}…`);
    this.setProgress(0.05);

    const form = new FormData();
    form.append("photo", this.file as File);
    ...
    const accepted = (await resp.json()) as AvatarJob;
  ...
  private pollJob(jobId: string, name: string): Promise<AvatarApplyPayload> {
    const started = performance.now();
    return new Promise((resolve, reject) => {
      const tick = async () => {
        try {
          ...
          const payload = (await resp.json()) as { jobs: AvatarJob[] };
