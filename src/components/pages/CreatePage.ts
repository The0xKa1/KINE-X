...
export class CreatePage implements Page {
  ...
  private initFlow(): void {
    new ImportFlow({
      fileInput: $("#createFile") as HTMLInputElement,
      dropZone: $("#createDrop"),
      motionSelect: $("#createMotion") as HTMLSelectElement,
      startButton: $("#createStart") as HTMLButtonElement,
      applyButton: $("#createApply") as HTMLButtonElement,
      segmentButton: $("#createSegment") as HTMLButtonElement,
  ...
  private syncSteps(state: ImportFlowState): void {
    ...
    const map: Record<Exclude<ImportFlowState, "error">, { done: number; active: number }> = {
    ...
    this.el.querySelectorAll<HTMLElement>(".create-steps li").forEach((li, index) => {
