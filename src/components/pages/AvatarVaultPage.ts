...
import {
  ...
  type AvatarPreviewPoseId,
} from "../../core/avatar/AvatarPreviewPoses.js";
import { THREE } from "../../core/three-compat.js";

interface AvatarVaultPageOptions {
  el: HTMLElement;
  client: AvatarRegistryClient;
}

type VaultLoadState = "idle" | "loading" | "ready" | "offline" | "error";

const PREVIEW_TARGET_Y = 1.05;
const MIN_CAMERA_DISTANCE = 2.4;
const MAX_CAMERA_DISTANCE = 7.2;

export class AvatarVaultPage implements Page {
  el: HTMLElement;
  private readonly client: AvatarRegistryClient;
  private records: AvatarIdentityRecord[] = [];
  private loadState: VaultLoadState = "idle";
  private loadError = "";
  ...
  private previewPoseId: AvatarPreviewPoseId = "akimbo";
  private dragging = false;
  private pointerX = 0;
  private pointerY = 0;
  private readonly lifecycle = new AsyncGenerationGuard();
  private readonly renameDrafts = new AvatarRenameDraftStore();

  constructor(options: AvatarVaultPageOptions) {
    this.el = options.el;
  ...
  private bindShell(): void {
    const form = this.requireElement<HTMLFormElement>("#avatarVaultUpload");
    const fileInput = this.requireElement<HTMLInputElement>("#avatarVaultFile");
    const canvas = this.requireElement<HTMLCanvasElement>("#avatarPreviewCanvas");
    const poseControls = this.requireElement<HTMLElement>(".avatar-preview-poses");
    ...
    fileInput.addEventListener("change", () => {
      const label = this.requireElement<HTMLElement>("#avatarVaultFileLabel");
      label.textContent = fileInput.files?.[0]?.name ?? "选择全身照片";
    });
    poseControls.addEventListener("click", (event) => {
      const target = event.target;
      if (!(target instanceof Element)) return;
      const button = target.closest<HTMLButtonElement>("[data-preview-pose]");
      if (!button || button.disabled) return;
      const poseId = button.dataset.previewPose as AvatarPreviewPoseId;
  ...
  private restartWatch(showLoading = false, generation = this.lifecycle.capture()): void {
  ...
  private async uploadSelectedFile(): Promise<void> {
    if (this.uploading) return;
    const fileInput = this.requireElement<HTMLInputElement>("#avatarVaultFile");
    const nameInput = this.requireElement<HTMLInputElement>("#avatarVaultName");
    ...
    try {
      ...
      fileInput.value = "";
      nameInput.value = "";
      this.requireElement<HTMLElement>("#avatarVaultFileLabel").textContent = "选择全身照片";
  ...
  private renderGallery(): void {
    ...
    const list = this.requireElement<HTMLElement>("#avatarVaultList");
    const count = this.requireElement<HTMLElement>("#avatarVaultCount");
    ...
    if ((this.loadState === "offline" || this.loadState === "error") && this.records.length === 0) {
      ...
      list.querySelector<HTMLButtonElement>("[data-retry-list]")?.addEventListener("click", () => this.restartWatch(true));
      return;
    }
    ...
    if (this.records.length === 0) {
      ...
      list.querySelector<HTMLButtonElement>("[data-focus-upload]")?.addEventListener("click", () => {
        this.requireElement<HTMLInputElement>("#avatarVaultFile").click();
      });
      return;
    }
    ...
    list.querySelector<HTMLButtonElement>("[data-retry-list]")?.addEventListener("click", () => this.restartWatch());
    this.bindCardActions(list);
    this.restoreRenameDraftFocus();
  }

  private renderCard(record: AvatarIdentityRecord, index: number): string {
  ...
  private bindCardActions(list: HTMLElement): void {
    list.querySelectorAll<HTMLButtonElement>("[data-select-avatar]").forEach((button) => {
    ...
    list.querySelectorAll<HTMLButtonElement>("[data-replacement-upload]").forEach((button) => {
      button.addEventListener("click", () => this.requireElement<HTMLInputElement>("#avatarVaultFile").click());
    });
    list.querySelectorAll<HTMLButtonElement>("[data-rename-avatar]").forEach((button) => {
      button.addEventListener("click", () => {
        ...
        this.renderGallery();
        this.el.querySelector<HTMLInputElement>("[data-rename-input]")?.focus();
      });
    });
    list.querySelectorAll<HTMLButtonElement>("[data-delete-avatar]").forEach((button) => {
      button.addEventListener("click", () => {
        if (this.renamingId) this.renameDrafts.finish(this.renamingId);
        this.deletingId = button.dataset.deleteAvatar ?? null;
        this.renamingId = null;
        this.renderGallery();
        this.el.querySelector<HTMLButtonElement>("[data-confirm-delete]")?.focus();
      });
    });
    list.querySelectorAll<HTMLButtonElement>("[data-cancel-rename]").forEach((button) => {
      button.addEventListener("click", () => {
        if (this.renamingId) this.renameDrafts.finish(this.renamingId);
        this.renamingId = null;
        this.renderGallery();
    ...
    list.querySelectorAll<HTMLButtonElement>("[data-cancel-delete]").forEach((button) => {
      button.addEventListener("click", () => {
        this.deletingId = null;
        this.renderGallery();
      });
    });
    list.querySelectorAll<HTMLFormElement>("[data-rename-form]").forEach((form) => {
      form.addEventListener("submit", (event) => {
        event.preventDefault();
    ...
    list.querySelectorAll<HTMLButtonElement>("[data-confirm-delete]").forEach((button) => {
      button.addEventListener("click", () => {
  ...
  private async renameAvatar(avatarId: string, name: string): Promise<void> {
  ...
  private async deleteAvatar(avatarId: string): Promise<void> {
    const generation = this.lifecycle.capture();
    if (!this.lifecycle.isCurrent(generation)) return;
    try {
  ...
  private async mountPreview(record: AvatarIdentityRecord): Promise<void> {
    const generation = this.lifecycle.capture();
    if (!this.lifecycle.isCurrent(generation)) return;
    this.disposePreview();
    ...
    try {
      ...
      this.requireElement<HTMLElement>("#avatarPreviewState").hidden = true;
  ...
  private syncPreviewPoseControls(): void {
    const supportsPose = Boolean(this.previewAvatar?.supportsReusableMotion);
    this.el.querySelectorAll<HTMLButtonElement>("[data-preview-pose]").forEach((button) => {
    ...
    const status = this.requireElement<HTMLElement>("#avatarPreviewPoseStatus");
    if (supportsPose) {
  ...
  private renderPreviewMetadata(record: AvatarIdentityRecord): void {
    const meta = this.requireElement<HTMLElement>("#avatarPreviewMeta");
    meta.innerHTML = `
  ...
  private renderPreviewStandby(title: string, detail: string, isError = false): void {
    const state = this.requireElement<HTMLElement>("#avatarPreviewState");
    state.hidden = false;
    state.classList.toggle("is-error", isError);
    state.innerHTML = `<strong>${escapeHtml(title)}</strong><span>${escapeHtml(detail)}</span>`;
    this.requireElement<HTMLElement>("#avatarPreviewMeta").innerHTML = `
      <span>IDENTITY</span><b>未选择</b>
      <span>RENDER</span><b>${isError ? "UNAVAILABLE" : "STANDBY"}</b>`;
  }

  private setNotice(message: string, isError: boolean): void {
    const notice = this.requireElement<HTMLElement>("#avatarVaultNotice");
    notice.hidden = false;
    notice.classList.toggle("is-error", isError);
    notice.textContent = message;
  }

  private syncUploadButton(): void {
    const button = this.requireElement<HTMLButtonElement>("#avatarVaultSubmit");
    button.disabled = this.uploading;
    button.textContent = this.uploading ? "上传中…" : "开始重建";
  }

  private captureRenameDraft(): void {
    if (!this.renamingId) return;
    const input = this.el.querySelector<HTMLInputElement>("[data-rename-input]");
    if (!input) return;
  ...
  private restoreRenameDraftFocus(): void {
    if (!this.renamingId) return;
  ...
  private requireElement<T extends Element>(selector: string): T {
    const element = this.el.querySelector<T>(selector);
    if (!element) throw new Error(`[AvatarVaultPage] missing ${selector}`);
    return element;
  }
}

function isPreviewReady(record: AvatarIdentityRecord): boolean {
