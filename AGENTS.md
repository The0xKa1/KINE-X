...
- **`src/components/pages/AvatarVaultPage.ts` + `src/core/avatar/AvatarPreviewPoses.ts`** — server-backed identity management and 3DGS preview. Reusable identities can be shown with four preview-only one-frame poses; these never mutate identity assets or training motions, while legacy combined identities remain on their original pose.
