...
Vanilla TypeScript SPA. No framework, no bundler, zero `package.json` dependencies. Rendering is **real Three.js** (WebGL): `src/core/three-compat.ts` is a facade that re-exports the THREE pieces the app uses as a single `THREE` object; `MotionStage` renders a cylinder-bone / sphere-joint skeleton plus an optional SMPL-X `MeshClip` (10 475 vertices, baked by the import backend) on the same canvas. Seeds with an `avatarUrl` also get a live 3DGS avatar (`src/core/avatar/GaussianAvatar.ts` — KINEXGS1 binary, per-gaussian LBS in the vertex shader + CPU depth sort). The avatar is **not a seed** — it is a fourth stage display mode (`MotionMode "avatar"`, 分身 button shown only when the current seed has an `avatarUrl`; `main.ts` `syncAvatarModeButton` hides it otherwise and falls back avatar→coach). Mode visibility: avatar mode = splats only (SMPL-X envelope acts as placeholder until the bin loads), coach/stress = envelope (avatar hidden), mesh = wireframe + rig.
...
- `backend/` :8765 — SAM 3D Body video import (FastAPI + GPU). `POST /import/video` (multipart; optional `startSec` / `endSec` slicing, `motion`, `targetFps`, `name`), `GET /import/jobs`, `GET /healthz`. Also `POST /import/avatar` — async photo → 3DGS avatar import (LHM exporter subprocess + `backend/alignment.py` stage-space bake; 202 + poll `GET /import/jobs` where records carry `kind: "video"|"avatar"`; `AVATAR_EXPORT_STUB=1` skips the GPU export for testing). Video artifacts land in `public/coach_clips/jobs/<jobId>/`, avatar artifacts in `public/coach_clips/jobs/avatar/<jobId>.{bin,json}` — both served by the frontend's own static server. Heavy deps (torch, sam-3d-body, ffmpeg) are **not** in `backend/requirements.txt` — see `backend/README.md`.
...
- `#/create` has two branches: 视频教练 (`ImportFlow` → `POST /import/video`) and 照片分身 (`AvatarImportFlow` → `POST /import/avatar` + poll `/import/jobs`; result attaches to the target seed as `avatarUrl`, v1 fixed to ugc-squat). `?mockAvatar=1` fakes the avatar backend locally for UI bring-up.
