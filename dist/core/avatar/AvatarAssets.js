...
export function parseGaussianMotion(buffer             )                      {
...
export function parseLegacyGaussianAsset(buffer             )                      {
...
export function progressFrame(progress        , frameCount        )         {
...
function readFloat32(cursor              , length        )               {
...
function readInt16(cursor              , length        )             {
...
function readUint8(cursor              , length        )             {
...
function requireJointCount(joints        )       {
...
function normalizeQuaternions(source              )               {
  let allUnit = true;
...
function parseStageTransform(value         )                                                      {
  ...
  const rowMajor = rotation.flat()             ;
  requireFinite(rowMajor            , "stage rotation");
  requireFinite(offset            , "stage translation");
  const translation = new Float32Array(offset            );
  const linear = new Float32Array(9);
  for (let row = 0; row < 3; row++) {
    for (let column = 0; column < 3; column++) {
      linear[column * 3 + row] = scale * (rowMajor[row * 3 + column]          );
    }
  }
  return { linear, translation };
}

function validateHierarchy(parents            )             {
