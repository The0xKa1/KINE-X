...
`index.html`：浏览器入口，加载 `src/styles.css` 与构建后的 `dist/main.js`；含 boot overlay、四个页面容器（`#page-library` / `#page-train` / `#page-report` / `#page-create`）、两个全局抽屉与两个模态；importmap 把 `@mediapipe/tasks-vision` 映射到 `public/mediapipe/`，把 `three` 映射到 `public/three/`。
...
`dist/`：构建产物，不手工修改。
`backend/`：SAM 3D Body 视频导入服务（FastAPI，:8765）。
...
`src/core/Router.ts`：`#/` 动作库、`#/train/:seedId` 训练舱、`#/report/:sessionId?` 报告、`#/create` 创作；未知路由回退 `#/`。
...
`src/components/pages/CreatePage.ts`：视频 → 虚拟教练四步向导，驱动 `ImportFlow`。

## src 总览

`src/main.ts`：组合根；装配 EventBus、帧缓存、Stage、RealtimeStream、SessionGate、Router 与四个页面、全部 UI 组件、MediaPipe、Calibration、Scoring、SessionRecorder / SessionArchive、AI Coach；启动时驱动 BootSequence、水合 clip 与 mesh、自愈缩略图、再水合已导入 jobs。
...
`src/styles.css`：仅含 `@import` 入口（21 个分文件）。
...
`styles/library.css` / `styles/report.css` / `styles/create.css`：三个页面的专用样式。
...
`public/coach_clips/`：`single_leg_squat.json`（118 帧）+ `single_leg_squat.mesh.{bin,faces.bin,meta.json}` + `single_leg_squat_frames`（**死符号链接**，缩略图由 `healTimelineThumbnails` 自愈）+ `jobs/`（导入产物，已 gitignore）。
