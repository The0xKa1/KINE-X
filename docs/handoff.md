...
> 最后更新：2026-07-18 深夜（**3DGS 实时数字人已上线**：方案 A 全链路打通）。
...
**3DGS 实时数字人（方案 A）✅ 已完成（2026-07-18）**：管线全部打通并验收（三视角截图、mesh 模式、npm run check、tsc 回到基线 11）。关键资产与工具：
...
- **对齐重烘**：`.deploy-tmp/avatar-lab/solve_alignment.py`（Kabsch 关节轨迹拟合 + 变换预烘进 bin + 按 clip 时长裁剪帧）。**任何新分身都必须跑这一步**，否则位置/朝向/时间全错。
...
**已知余量（可选优化）**：top-4 截断在面部区域有 ~7mm 极值误差（v2 可上 top-8/全 55）；排序 16bit 索引上限 N≤65536；分身目前绑定同一套 squat 动作（换新动作 = 新 smplx 参数序列重导）。
