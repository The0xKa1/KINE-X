...
复赛产品化打磨阶段，多页面骨架已落地。
当前前端与静态资产版本为 `0.1.1`（版本真源为 `package.json`）。
...
guardrails 同时要求页面显示版本、入口 CSS/JS、23 个 CSS `@import` 与全部相对 JS module import 和 `package.json` 版本一致。
`tsc --noEmit` 目前 12 个已知诊断（`noUncheckedIndexedAccess` 严格性），仅作开发参考，未入门禁。
...
照片 → 可复用身份 × 视频动作 → 浏览器实时 3DGS 数字分身全链路已通，并经远端验收（持久化身份库、环绕/缩放预览、导入先返回后绑定、训练回放、WebM 导出、软删除后已完成训练保留；动作尺度固定米制 `scale=1.0`，多视角无压缩团块）。
