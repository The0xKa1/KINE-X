...
训练舱为三舱结构：左舱「现实镜像」摄像头水平镜像 + 2D 骨骼贴合层；中舱「数字分身」播放真人教练视频（`CoachVideo`，scrub / tempo / 播放态与 RealtimeStream 同步，支持 front / side / top 多角度视频源，无视频时显示空态 + 创作工坊入口），SYNC 巨数与风险徽章位于中舱；右舱「结构蓝图」Three.js 3D 教练，渲染模式（教练 / 骨骼 / 应力）与 front-side-top 视角按钮位于该舱，拖拽旋转 / 滚轮缩放 / 双击复位 / 阻尼飞行运镜，静置 3s 后 idle 慢摆；mesh 模式骨架 + 线框蓝图并存（骨骼向骨盆轴线收缩 0.72 避免与包络相缠）；stress 关节 `#ff5500` 低频脉冲。
