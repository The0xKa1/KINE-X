...
复赛产品化打磨阶段，多页面骨架已落地。
当前前端与静态资产版本为 `0.1.11`（版本真源为 `package.json`）。
...
视频 → CoachClip 导入链路走 SAM 3D Body 后端（:8765）；可选 MLLM 分片由浏览器直连用户配置的 OpenAI-compatible API。
评分、用户标定、Session 门禁（倒计时 / OK 手势）、结算页、Session 历史存档、AI 教练流式输出已联通。
帧流 WebSocket 客户端已就绪（自动重连 + 心跳），但符合 `FRAME_STREAM` 的真实后端不存在于本仓库。
本地 `npm run dev` 即可完整演示；three、MediaPipe、Archivo Black、JetBrains Mono 与完整 Noto Sans SC 均从本地 `public/` 加载，无外部运行时资产依赖。
...
boot 完成后进入初始路由，默认 `#/`。
`?backend=` / `?ws=` / `?mode=` 查询参数保持原有行为（启动读取一次）。
...
内置种子为 squat 与 ugc-squat（均绑定真实 CoachClip）；更早的 deadlift / baduanjin / street / basketball 已下架，其评分权重以 `MOTION_METRIC_TEMPLATES` 形式保留给导入动作复用。
...
`SessionArchive`（`localStorage` key `kinex.sessions.v1`，最多 20 场）驱动报告页与动作库统计（场次 / 最好成绩 / 训练记录）。`remove(id)` 精确删除单场；动作库显示全部保留记录并提供删除按钮，报告页也可删除当前场次，均需确认，删除后统计与趋势立即重算。
...
MediaPipe 资产走本地 `public/mediapipe/`；三套界面字体走本地 `public/fonts/`，其中 Noto Sans SC 保留完整中文字形与 100–900 可变字重。
...
`scripts/build.mjs` 做 TypeScript 类型擦除（Node `stripTypeScriptTypes`），并按 `package.json` 版本为构建产物内所有相对 `.js` import 追加 query；仍不打包、不转译。
...
guardrails 同时要求页面显示版本、入口 CSS/JS、22 个 CSS `@import` 与全部相对 JS module import 和 `package.json` 版本一致。
`tsc --noEmit` 目前 8 个已知诊断（`noUncheckedIndexedAccess` 严格性），仅作开发参考，未入门禁。
...
本地字体包约 7.5MB，主要来自完整 Noto Sans SC；部署时必须连同 `public/fonts/` 与三份 OFL 许可证一起发布，guardrail 会拒绝远程 Google Fonts 引用、缺失或无效的 WOFF2。
...
`tsc --noEmit` 清零（当前 8 个）后纳入 `npm run check` 门禁。
...
照片 → 可复用身份 × 视频动作 → 浏览器实时 3DGS 数字分身全链路已通，并经远端验收（持久化身份库、环绕/缩放预览、导入先返回后绑定、训练回放、软删除后已完成训练保留；动作尺度固定米制 `scale=1.0`，多视角无压缩团块）。训练结束后的 DNA/WebM 视频导出已移除。
