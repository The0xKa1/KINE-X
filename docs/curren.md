...
训练舱：左舱「现实镜像」摄像头水平镜像 + 2D 骨骼贴合层；右舱「全息标准舱」Three.js 3D 教练，拖拽旋转 / 滚轮缩放 / 双击复位 / front-side-top 阻尼飞行运镜，静置 3s 后 idle 慢摆；mesh 模式骨架 + 线框蓝图并存（骨骼向骨盆轴线收缩 0.72 避免与包络相缠）；stress 关节 `#ff5500` 低频脉冲。
...
结算模态 `ResultsScreen` 展示 count-up 总分、勋章、四宫格与关节报告表，并在 `open()` 时把整场（含 `SessionSummary`）写入 `SessionArchive`。
