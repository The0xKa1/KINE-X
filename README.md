...
| 资产缓存一致性 | 前端 `0.1.11`：入口、CSS 依赖、本地字体和完整本地业务 ES module 图统一版本；可重烘的身份 / 动作 / 预览 URL 按文件状态自动换版 |
...
填写后点击「测试两项连接」：页面会用极小请求分别验证 MLLM 的图片输入 + JSON 模式，以及赛后模型的 SSE 流式输出，并显示分项延迟或 HTTP/CORS 错误。测试请求可能产生少量服务商计费。

配置保存在当前浏览器的 `localStorage`，视频关键帧和赛后摘要都由浏览器直接发往该 API，不经过 KINE//X 服务器。服务商必须允许浏览器跨域请求；建议使用限额且可随时撤销的 Key，不要在共享设备保存生产密钥。

赛后报告首次生成的是 120 字内的单段诊断；报告页后续追问改用独立对话提示词，会区分实际测量与生物力学推断，并在疼痛或损伤话题中提示停止训练及寻求专业评估。每次请求只携带最近 4 个完整问答轮次，历史正文合计最多 4000 字，单次问题最多 500 字。
